...
namespace Playground.Charting
{
    public class DrawPolar : DrawBase
    {
        ...
        public override void Draw(IEnumerable<DataRecord> records, bool pointsVisible, Color lineColor)
        {
            ...
            if (!styleLoaded)
            {
                polarAxis = InjectedChart.ViewPolar.Axes[0];
                ConfigurePolarAxis(polarAxis);
                ConfigureView(InjectedChart.ViewPolar);
                styleLoaded = true;
            }
            ...
            if (points.Count > 0)
            {
                ...
                for (int i = 0; i < num_sectors; i++)
                {
                    ...
                    meanMarker.Amplitude = sector_points
                        .Select(p => p.Amplitude).Max() * 1.09;
                    meanMarker.Label.Text = $"Mean: {
                        Double.Round(
                            sector_points.Select(p => p.Amplitude).Average(), 2
                        )
                    }";
                ...
                data.PointStyle.Color1 = lineColor - Color.FromArgb(0, 115, 115, 115);
                data.LineStyle.Color = lineColor;
                
                double rMargin = (maxR - minR) * 0.1;
                rMargin = rMargin == 0 ? 1 : rMargin;
                polarAxis?.MinAmplitude = minR - rMargin;
                polarAxis?.MaxAmplitude = maxR + rMargin;
            }
        }

        private static void ConfigurePolarAxis(AxisPolarBase axis)
        {
            axis.Title.Shadow.Style = TextShadowStyle.Off;
            axis.Title.Color = Colors.Black;
            axis.Units.Color = Colors.Black;
            axis.MinorDivTickStyle.Color = Colors.Black;
            axis.MajorDivTickStyle.Color = Colors.Black;
            axis.AxisColor = Color.FromRgb(103, 140, 171);
            axis.ScaleNibs.Color = Color.FromRgb(95, 177, 245);
            axis.Title.Text = "'R' Values";
            axis.MajorGrid.LabelsColor = Colors.DarkSlateGray;
            axis.MinorGrid.LabelsColor = Colors.DarkSlateGray;
            axis.GridAngular.LabelsColor = Colors.DarkSlateGray;
        }

        private static void ConfigureView(ViewPolar view)
        {
            view.GraphBackground.Style = RectFillStyle.None;
            view.AutoSizeMargins = false;
            view.Margins = new System.Windows.Thickness(50, 50, 50, 50);
        }

        private static void ConfigureSector(Sector sector)
        {
